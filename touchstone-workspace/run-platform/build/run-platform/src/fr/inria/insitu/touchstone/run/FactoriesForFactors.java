...
class FactorFactory extends BasicFactory {

	protected FactorFactory(String name) {
		super(name);
	}
...
public class FactoriesForFactors {

	private static final Logger LOG = Logger.getLogger(FactoriesForFactors.class.getName());
	/**
	...
	private static FactoriesForFactors instance = null;
	
	public static FactoriesForFactors getInstance() {
		if(instance == null) { 
			instance = new FactoriesForFactors();
	...
	private void addCreators(Properties p) {
		for (int i = 0; i < 1000; i++) {
            ...
            if(factorFactory == null) {
            	factorFactory = new FactorFactory(nameFactor);
            }
...
//            	System.out.println("add ("+nameProperty+", "+classProperty+") to "+nameFactor);
	...
	public static FactorFactory getFactor(String idFactor) {
		BasicFactory factorFactory = BasicFactory.getFactory(idFactor);
        if(factorFactory instanceof FactorFactory) {
        	return (FactorFactory)factorFactory;
        }
        return null;
	}
